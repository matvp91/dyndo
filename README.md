...
into a tiny JSON descriptor; the server then generates DASH and HLS manifests and
serves CMAF segments *on the fly*, straight from the original files via HTTP
byte-range reads.
...
- **[Explanation](https://matvp91.github.io/dyndo/explanation/thin-pointer.html)** —
  the thin-pointer design, bounded-memory parsing, and one source / two protocols.
...
flowchart LR
    ...
    S -->|/hls/index.m3u8| H[HLS playlists]
    S -->|/&lt;repr&gt;/&lt;time&gt;.m4s| G[shared CMAF segments]
```
