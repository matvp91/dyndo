//! Pure segment arithmetic: grouping a track's raw CMAF fragments into
//! served segments. No I/O; everything here operates on [`Segment`] slices.

use crate::segment::Segment;

/// Group a track's raw CMAF fragments into served segments. Splice points
/// (`boundaries_ms`) partition the fragments — a served segment never spans
/// one — and within each partition whole fragments are accumulated greedily
/// until a group reaches `min_length_ms`. The final group of a partition may
/// be shorter (short tail). A `min_length_ms` of 0 disables grouping.
/// Fragments are contiguous byte ranges, so a group is the first fragment's
/// offset with summed sizes and durations. All comparisons are exact u128
/// integer math (`units * 1000` vs `ms * timescale`), so tracks with
/// different timescales make identical grouping decisions and stay aligned.
pub(crate) fn group_segments(
    ...
    let min_target = min_length_ms as u128 * timescale as u128;
    ...
    let cuts = snap_cuts(&cum, timescale, boundaries_ms);
...
mod tests {
    ...
    /// A contiguous fragment run: `durations` in timescale units, each
    /// fragment 10 bytes, starting at byte 100 (mirroring data after an init
    /// segment).
    fn frags(durations: &[u64]) -> Vec<Segment> {
        let mut offset = 100;
        durations
            .iter()
            .map(|&d| {
    ...
    fn the_track_tail_may_be_shorter_than_the_minimum() {
        let raw = frags(&[172800, 172800, 122400]);
        let out = group_segments(&raw, 90000, &[], 3000);
        assert_eq!(out.len(), 2);
    ...
    fn a_conditioning_fragment_before_a_splice_becomes_a_short_segment() {
        ...
        let raw = frags(&[172800, 172800, 10800, 162000, 172800]);
    ...
    fn an_exact_tie_snaps_earlier() {
        ...
        let raw = frags(&[180000, 180000, 180000]);
    ...
    fn video_and_text_group_to_identical_boundary_times() {
        ...
        // across tracks.
        let video = frags(&[172800, 172800, 10800, 162000, 172800]);
        let text = frags(&[1920, 1920, 120, 1800, 1920]);
